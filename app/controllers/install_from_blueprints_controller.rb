class InstallFromBlueprintsController < ApplicationController
  ...
  def new
    ...
    if @install_from_blueprint.blueprint.empty?
      redirect_to installer_path(search: params[:search], page: params[:page], tags: params[:tags]),
        alert: "Unable to install #{install_from_blueprint_params[:software_title] || 'software'}. Cannot load from repository #{install_from_blueprint_params[:repository_url]}."
